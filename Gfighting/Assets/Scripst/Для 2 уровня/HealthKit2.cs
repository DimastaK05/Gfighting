...
public class HealthKit2 : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        {
            if (other.TryGetComponent(out PlayerManager2 player))
            {

                PlayerManager2.playerHealth = Mathf.Min(PlayerManager2.playerHealth + healAmount, 100f);


                player.Bar.fillAmount = PlayerManager2.playerHealth / 100f;

                // Если нужно обновить текстовое отображение
                if (player.playerHealthText != null)
                    player.playerHealthText.text = $"HP: {Mathf.CeilToInt(PlayerManager2.playerHealth)}/100";
