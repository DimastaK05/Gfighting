    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (other.CompareTag("Player"))
        {
            if (other.TryGetComponent(out PlayerManager player))
            {
                ...
                PlayerManager.playerHealth = Mathf.Min(PlayerManager.playerHealth + healAmount, 100f);

     
                player.Bar.fillAmount = PlayerManager.playerHealth / 100f;

                // Если нужно обновить текстовое отображение
                if (player.playerHealthText != null)
                    player.playerHealthText.text = $"HP: {Mathf.CeilToInt(PlayerManager.playerHealth)}/100";
